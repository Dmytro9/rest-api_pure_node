...
handlers.accountCreate = function (data, callback) {
  ...
  if (data.method == 'get') {
    ...
    var templateData = {
      ...
      'head.description': 'Signup is easy and only a few seconds',
...
handlers.accountDeleted = function (data, callback) {
  ...
  if (data.method == 'get') {
    ...
    var templateData = {
      ...
      'head.description': 'Yrou account has been deleted',
        ...
        _data.read('users', phone, function (err, userData) {
          if (!err && userData) {
            _data.delete('users', phone, function (err) {
              if (!err) {
                // Delete each the checks associated with the user
                    ...
                    _data.delete('checks', checkId, function (err) {
                      ...
                      if (checksDeleted == checksToDelete) {
                        if (!deletionErrors) {
                          ...
                          callback(500, { 'Error': 'Errors encountered while attempting to delete alll of the user\'s checks. All checks may not have been deleted from the system successfully' });
                        }
                      }
                    });
                  });
    ...
    _data.read('tokens', token, function (err, tokenData) {
      if (!err && tokenData) {
        ...
        _data.read('users', userPhone, function (err, userData) {
          if (!err && userData) {
            var userChecks = typeof (userData.checks) == 'object' && userData.checks instanceof Array ? userData.checks : [];
            // Verify that that the user has less than the number of max-checks-per-user
