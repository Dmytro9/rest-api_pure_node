...
cli.responders.help = function () {
  var commands = {
    ...
    'list users': 'Show a list of the register (undeleted) users in the system',
    'more user info --{userId}': 'Show details of the specific user',
    'list checks --up --down': 'Show a list of all the active checks in the system, including their state. The "--up" and "--down" flags are both optional',
    'more check info --{checkId}': 'Show details of a specified check',
    'more logs': 'Show a list of all the log files available to be read (compressed and uncompressed)',
...
cli.centered = function (str) {
  ...
  // Put in left padded spacws before the string itself
  var line = '';
    ...
    if (!err && checkIds && checkIds.length > 0) {
      ...
      checkIds.forEach(function (checkId) {
        _data.read('checks', checkId, function (err, checkData) {
          ...
          var stateOrUknown = typeof (checkData.state) == 'string' ? checkData.state : 'unknown';
          // If the user has specified the state, or hasn't specified any state, include the current check accordingly
          if (lowerString.indexOf('--' + state) > -1 || (lowerString.indexOf('--down') == -1) && lowerString.indexOf('--up') == -1) {
            var line = 'ID: ' + checkData.id + ' ' + checkData.method.toUpperCase() + ' ' + checkData.protocol + '://' + checkData.url + ' State: ' + stateOrUknown;
            console.log(line);
            cli.verticalSpace();
          }
        });
